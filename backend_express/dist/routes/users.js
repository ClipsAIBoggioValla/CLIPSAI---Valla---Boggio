...
usersRouter.get('/me', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const u = r.rows[0];
        return res.json({ id: u.id, email: u.email, full_name: u.full_name ?? null, name: u.full_name ?? null, avatar_url: u.avatar_url ?? null, theme_preference: u.theme_preference ?? 'dark', created_at: new Date(u.created_at).toISOString() });
    }
    catch (err) {
        ...
        try {
            ...
            if (fallback.rows.length > 0) {
                ...
                return res.json({ id: u.id, email: u.email, full_name: u.full_name ?? null, name: u.full_name ?? null, avatar_url: null, theme_preference: 'dark', created_at: new Date(u.created_at).toISOString() });
            }
        }
        catch { }
...
function buildUpdateHandler() {
    return async (req, res) => {
        const userId = req.user.id;
        const body = req.body;
        const fullNameRaw = (body.full_name ?? body.nombre);
        const emailRaw = body.email;
        const avatarRaw = body.avatar_url;
        const themeRaw = body.theme_preference;
        ...
        if (hasAvatar) {
            let normalized = avatarRaw ?? null;
            if (normalized !== null && normalized.trim() === '')
                normalized = null;
            if (normalized !== null) {
                if (normalized.length > 500)
                    return res.status(422).json({ detail: 'avatar_url debe tener máximo 500 caracteres' });
                if (!isValidAvatarUrl(normalized))
                    return res.status(422).json({ detail: 'avatar_url debe ser una URL http(s)' });
            }
        ...
        try {
            ...
            try {
                ...
                if (fallbackQ.includes('SET  WHERE'))
                    throw e;
...
async function handleChangePassword(req, res) {
    ...
    try {
        ...
        if (r.rows.length === 0)
            return res.status(401).json({ detail: 'Usuario no encontrado' });
        const hashed = r.rows[0].hashed_password;
