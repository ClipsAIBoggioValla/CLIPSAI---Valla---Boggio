...
authRouter.post('/registro', async (req, res) => {
    ...
    const nameStr = (typeof full_name === 'string' ? full_name : typeof fullName === 'string' ? fullName : null);
    if (!isValidEmail(emailStr))
        return res.status(422).json({ detail: 'email inválido' });
    if (typeof pwdStr !== 'string' || pwdStr.length < 8 || pwdStr.length > 128)
        return res.status(422).json({ detail: 'password debe tener entre 8 y 128 caracteres' });
    if (nameStr !== null && typeof nameStr === 'string' && nameStr.length > 100)
    ...
    try {
        ...
        if (exists.rows.length > 0)
            return res.status(409).json({ detail: 'El email ya está registrado' });
        const hashed = await bcrypt.hash(truncate72(pwdStr), 10);
        const r = await pool.query('INSERT INTO usuarios (email, hashed_password, full_name) VALUES ($1,$2,$3) RETURNING id, email, full_name, created_at, updated_at', [emailStr, hashed, nameStr ?? null]);
        const u = r.rows[0];
        return res.status(201).json({ id: u.id, email: u.email, full_name: u.full_name ?? null, created_at: new Date(u.created_at).toISOString(), updated_at: new Date(u.updated_at ?? u.created_at).toISOString() });
    }
    catch (err) {
        const e = err;
        if (e?.code === '23505')
...
async function handleLogin(emailRaw, pwdRaw, res) {
    ...
    try {
        ...
        if (r.rows.length === 0)
            return res.status(401).json({ detail: 'Credenciales inválidas' });
        const row = r.rows[0];
        const ok = await bcrypt.compare(truncate72(password), row.hashed_password);
        if (!ok)
            return res.status(401).json({ detail: 'Credenciales inválidas' });
...
authRouter.get('/me', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const u = r.rows[0];
        return res.json({ id: u.id, email: u.email, full_name: u.full_name ?? null, created_at: new Date(u.created_at).toISOString(), updated_at: new Date(u.updated_at ?? u.created_at).toISOString() });
    }
    catch (err) {
