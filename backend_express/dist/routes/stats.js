...
statsRouter.get('/summary', authMiddleware, async (req, res) => {
    const userId = req.user.id;
    const client = await pool.connect();
    try {
        const totalVideosRes = await client.query('SELECT COUNT(*)::int AS count FROM videos WHERE usuario_id = $1', [userId]);
        const total_videos = totalVideosRes.rows[0]?.count ?? 0;
      ...
      `, [userId]);
        ...
        const total_clips = Number(agg.total_clips ?? 0);
        const rawAvg = agg.avg_score;
        const avg_score = rawAvg !== null && rawAvg !== undefined ? Math.round(Number(rawAvg) * 10) / 10 : 0.0;
        const estimated_time_saved_minutes = total_clips * 15;
