...
async function runPublish(clipId, platform, caption, webhookOverride) {
        ...
        try {
            ...
            if (res.ok) {
                try {
                    ...
                    fakeId = String(data.id ?? data.post_id ?? fakeId);
                    fakeUrl = String(data.url ?? data.post_url ?? `${URLS[platform] ?? URLS.webhook}${fakeId}`);
                }
                catch { }
...
async function handlePublish(req, res) {
    ...
    const { platform, caption, webhook_override_url } = req.body;
    const plat = String(platform ?? '').trim().toLowerCase();
    if (!ALLOWED.has(plat))
        return res.status(422).json({ detail: `platform debe ser una de ${[...ALLOWED].join(', ')}` });
    if (caption !== undefined && caption !== null && typeof caption !== 'string')
        return res.status(422).json({ detail: 'caption debe ser string' });
    if (caption && String(caption).length > 500)
        return res.status(422).json({ detail: 'caption max 500 chars' });
    await ensurePublishColumns();
    const client = await pool.connect();
    try {
        const r = await client.query(`SELECT c.id, v.usuario_id FROM clips c JOIN jobs j ON c.job_id=j.id JOIN videos v ON j.video_id=v.id WHERE c.id=$1`, [clipId]);
        if (r.rows.length === 0)
            return res.status(404).json({ detail: 'Clip no encontrado' });
        if (String(r.rows[0].usuario_id) !== String(userId))
