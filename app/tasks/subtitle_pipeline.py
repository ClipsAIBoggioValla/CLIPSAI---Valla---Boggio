...
try:
    ...
    from app.models import Clip  # type: ignore
except ImportError:
...
def _resolve_video_path(clip: Clip, db: Session) -> str | None:
    ...
    if clip.video_id is not None:
        try:
            try:
                from app.models import Video  # type: ignore
            except ImportError:
                from backend_fastapi.app.models import Video
    ...
    if clip.job_id is not None:
        try:
            try:
                from app.models import Job, Video  # type: ignore
            except ImportError:
...
def run_subtitle_pipeline(clip_id: uuid.UUID | str) -> None:
    ...
    try:
        ...
        try:
            from app.services.ass_generator import write_ass_file  # type: ignore
            from app.services.ffmpeg_service import burn_subtitles  # type: ignore
            from app.services.whisper_service import extract_audio_wav, transcribe_wav  # type: ignore
        except ImportError:
            from backend_fastapi.app.services.ass_generator import write_ass_file
            from backend_fastapi.app.services.ffmpeg_service import burn_subtitles
