  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      <div className="max-w-2xl mx-auto px-4 py-8 sm:py-12">
        <Link to="/upload" className="text-sm text-slate-600 hover:text-white inline-flex items-center gap-1 mb-6">
          ← Volver a subir
        </Link>

        <div className="bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 shadow-sm rounded-2xl p-6 sm:p-8 shadow-xl">
          <h1 className="text-xl font-semibold">Estado del procesamiento</h1>
          <p className="text-sm text-slate-600 mt-1 font-mono break-all">Job ID: {jobId}</p>
          ...
          {job && (
            <div className="mt-6 space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-3 bg-white dark:bg-slate-800/60 rounded-xl px-4 py-4 border border-slate-200/70 dark:border-slate-800 shadow-sm shadow-slate-100/80">
              ...
              {job.status.toUpperCase() === 'PENDING' && (
                <div className="rounded-xl bg-amber-500/5 border border-amber-500/20 p-4 flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600 mt-1">Tu video está esperando a ser procesado. Esto se actualiza solo.</p>
                  </div>
                </div>
              )}
              ...
              {job.status.toUpperCase() === 'PROCESSING' && (
                <div className="rounded-xl bg-sky-500/5 border border-sky-500/20 p-4 flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600 mt-1">Puede tardar unos minutos. La página se actualiza automáticamente.</p>
                  </div>
                </div>
              )}
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-600 mt-1">
                          ...
                          {job.result_metadata?.engine && (
                            <span className="ml-2 text-xs text-slate-600">· motor: {job.result_metadata.engine}</span>
                          )}
                        </p>
                      </div>
                    </div>
                    ...
                    {clips.length > 0 && (
                      <div className="grid gap-3 sm:gap-4">
                        {clips.map((clip, idx) => (
                          <div
                            key={`${clip.titulo}-${idx}`}
                            className="rounded-xl bg-white dark:bg-slate-800/80 border border-slate-200/70 dark:border-slate-800 shadow-sm shadow-slate-100/80 p-4 sm:p-5 flex flex-col gap-3"
                          >
                            <div className="flex items-start justify-between gap-3">
                              ...
                              {typeof clip.score === 'number' && (
                                <span className="shrink-0 inline-flex items-center rounded-full bg-gradient-to-r from-violet-600 to-indigo-600/15 border border-brand-500/30 px-2.5 py-1 text-xs font-semibold text-violet-300">
                                  Score {clip.score.toFixed(2)}
                                </span>
                              )}
                            </div>

                            <p className="text-xs sm:text-sm font-mono text-slate-600 bg-white dark:bg-slate-900/70 rounded-lg px-3 py-2 border border-slate-200/70 dark:border-slate-800 shadow-sm shadow-slate-100/80">
                              Inicio: {clip.inicio} - Fin: {clip.fin}
                            </p>

                            {clip.transcript_preview && (
                              <p className="text-sm text-gray-300 leading-relaxed line-clamp-3 bg-white dark:bg-slate-900/40 rounded-lg px-3 py-2">
                                {clip.transcript_preview}
                              </p>
                            )}
              ...
              {job.status.toUpperCase() === 'FAILED' && (
                <div role="alert" className="rounded-xl bg-red-500/10 border border-red-500/30 p-4">
                  ...
                  <Link
                    to="/upload"
                    className="mt-4 inline-flex rounded-xl bg-white dark:bg-slate-800 hover:bg-gray-700 border border-slate-200/70 dark:border-slate-800 shadow-sm shadow-slate-100/80 text-white text-sm font-medium py-2.5 px-4 transition"
                  >
                    Intentar de nuevo
                  </Link>
                </div>
              ...
              <div className="pt-2 border-t border-slate-200 dark:border-slate-700 grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-slate-600">
