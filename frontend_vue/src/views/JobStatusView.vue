...
<template>
  <div class="min-h-screen bg-gray-950 text-white">
    <div class="max-w-2xl mx-auto px-4 py-8 sm:py-12">
      <RouterLink to="/upload" class="text-sm text-gray-400 hover:text-white inline-flex items-center gap-1 mb-6">← Volver a subir</RouterLink>

      <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 sm:p-8 shadow-xl">
        <h1 class="text-xl font-semibold">Estado del procesamiento</h1>
        <p class="text-sm text-gray-500 mt-1 font-mono break-all">Job ID: {{ jobId }}</p>
        ...
        <div v-if="!job && pollingStatus === 'polling' && !error" class="mt-8 flex flex-col items-center gap-3 py-8">
          <span class="h-8 w-8 animate-spin rounded-full border-2 border-violet-500/30 border-t-violet-500" />
          <p class="text-sm text-gray-400">Consultando estado...</p>
        </div>

        <div v-if="job" class="mt-6 space-y-6">
          <div class="flex flex-wrap items-center justify-between gap-3 bg-gray-800/60 rounded-xl px-4 py-4 border border-gray-800">
            ...
            <span v-else class="text-sm text-gray-400">{{ job.status }}</span>
          </div>
          ...
          <div v-if="job.status === 'PENDING'" class="rounded-xl bg-amber-500/5 border border-amber-500/20 p-4 flex gap-3">
            ...
            <div><p class="text-sm font-medium text-amber-200">En cola...</p><p class="text-sm text-gray-400 mt-1">Tu video está esperando a ser procesado. Esto se actualiza solo.</p></div>
          </div>
          ...
          <div v-if="job.status === 'PROCESSING'" class="rounded-xl bg-sky-500/5 border border-sky-500/20 p-4 flex gap-3">
            ...
            <div><p class="text-sm font-medium text-sky-200">Procesando video y generando clips...</p><p class="text-sm text-gray-400 mt-1">Puede tardar unos minutos. La página se actualiza automáticamente.</p></div>
          </div>
          ...
          <div v-if="job.status === 'COMPLETED'" class="space-y-4">
            ...
            <div class="rounded-xl bg-emerald-500/10 border border-emerald-500/30 p-4 flex gap-3">
              ...
              <div>
                ...
                <p class="text-sm text-gray-400 mt-1">{{ clips.length > 0 ? `${clips.length} clip${clips.length !== 1 ? 's' : ''} generado${clips.length !== 1 ? 's' : ''}` : 'Tus clips ya están listos.' }}<span v-if="job.result_metadata?.engine" class="ml-2 text-xs text-gray-500">· motor: {{ job.result_metadata.engine }}</span></p>
              </div>
            </div>

            <div v-if="clips.length > 0" class="grid gap-3 sm:gap-4">
              <div v-for="(clip, idx) in clips" :key="`${clip.titulo}-${idx}`" class="rounded-xl bg-gray-800/80 border border-gray-700 p-4 sm:p-5 flex flex-col gap-3">
                <div class="flex items-start justify-between gap-3">
                  <h3 class="text-sm sm:text-base font-semibold text-white leading-snug line-clamp-2">{{ clip.titulo }}</h3>
                  <span v-if="typeof clip.score === 'number'" class="shrink-0 inline-flex items-center rounded-full bg-violet-500/15 border border-violet-500/30 px-2.5 py-1 text-xs font-semibold text-violet-300">Score {{ clip.score.toFixed(2) }}</span>
                </div>
                <p class="text-xs sm:text-sm font-mono text-gray-400 bg-gray-900/70 rounded-lg px-3 py-2 border border-gray-800">Inicio: {{ clip.inicio }} - Fin: {{ clip.fin }}</p>
                <p v-if="clip.transcript_preview" class="text-sm text-gray-300 leading-relaxed line-clamp-3 bg-gray-900/40 rounded-lg px-3 py-2">{{ clip.transcript_preview }}</p>
                <p class="text-[11px] text-gray-500">Modo simulado — sin archivo recortado individual, timestamps informativos</p>
              </div>
            </div>
          ...
          <div v-if="job.status === 'FAILED'" role="alert" class="rounded-xl bg-red-500/10 border border-red-500/30 p-4">
            ...
            <RouterLink to="/upload" class="mt-4 inline-flex rounded-xl bg-gray-800 hover:bg-gray-700 border border-gray-700 text-white text-sm font-medium py-2.5 px-4 transition">Intentar de nuevo</RouterLink>
          </div>

          <div class="pt-2 border-t border-gray-800 grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-gray-500">
